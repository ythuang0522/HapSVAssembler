...
printf " -"
samtools mpileup -Auf $fasta $sortedbamfile | bcftools view -vcg - > snp.vcf
...
samtools view -h $workDir/bwa_result/$bamfile > $workDir/bwa_result/$bamfile.sam

#last awk is used for removing singleton paired read record
awk '$1!="@PG" && $1!="@SQ"'  $workDir/bwa_result/$bamfile.sam | cut -f 1,2,4,6,8,9,10 | awk '{if ($1==prevkey) { print prev "\n" $0 }; prevkey=$1; prev=$0}'> sam_summary.log
awk 'BEGIN{OFS="\t"} {print $4,$2,$6,$3,$5}' sam_summary.log > paired-align.dist
#read -p "wait.........."
r=`wc -l paired-align.dist | cut -d ' ' -f 1`
cut -f 1,4,6 $workDir/bwa_result/$bamfile.sam | sort -T $workDir/$TMPDIR -k 2,2 -n | awk '$2!= 0' | awk '$3!="*"' | grep 'S' | grep -v '@' > tmp
...
echo $workDir/bin/svDetector -z $insert_size -v $variance -l $read_length -r $r -b $b
